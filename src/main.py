...
def start_message(msg):
    bot.send_message(msg.chat.id, "Добро пожаловать! Пожалуйста, отправьте что-нибудь или нажмите на кнопку \"Информация\"", reply_markup=my_keyboard)


@bot.message_handler(content_types=["text"])
def send_text(message):
    if message.text.lower() == "информация":
        bot.send_message(message.chat.id, """Что я умею делать?
...
def handle_photo(message):
    ...
    answer = "ok"
    # print(count_faces(cv2.imread("../data/images/image2.jpg")))
